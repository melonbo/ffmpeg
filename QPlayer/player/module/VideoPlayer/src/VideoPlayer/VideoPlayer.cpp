...
VideoPlayer::VideoPlayer()
{
    ...
    mVolume = 1;
//    startPlay("F:/share/video/earth01.mp4");
    startPlay("rtsp://192.168.31.55:8554/1");
}
...
void VideoPlayer::readVideoFile()
{
    ...
    for (int i = 0; i < pFormatCtx->nb_streams; i++)
    {
        ...
        {
            videoStream = i;
            OUTPUT("video stream is %d", videoStream);
        }
        if (pFormatCtx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO  && audioStream < 0)
        {
//            audioStream = i;
            OUTPUT("audio stream is %d", audioStream);
        }
    }
    ...
    //        out_linesize = av_samples_get_buffer_size(NULL, audio_tgt_channels, av_get_bytes_per_sample(out_sample_fmt), out_sample_fmt, 1);
            ...
            OUTPUT("audio stream time base : %d / %d", mAudioStream->time_base.num, mAudioStream->time_base.den);
